...
def build_tensor_tqdm_gpt(data):
    ...
    for _, id in enumerate(tqdm(unique_ids)):  # 添加 tqdm 进度条
        ...
        label_data = data[data[:, 0] == id][:, -3].astype(float)
        basic_data = data[data[:, 0] == id][:, -2:-1].astype(float)
        for j in range(patient_data.shape[0] - TIME_STEPS + 1):
            ...
            basic_meta_data = torch.tensor(basic_data[j + TIME_STEPS - 1]).unsqueeze(0)
            basic_list.append(basic_meta_data)

    # 将列表转换为张量
    data_tensor = torch.cat(data_list, dim=0)
    label_tensor = torch.cat(label_list, dim=0)
    basic_tensor = torch.cat(basic_list, dim=0)

    return data_tensor, label_tensor, basic_tensor
...
def splited_tensor(data, train_rate, val_rate):
    ...
    data_tensor_train, label_tensor_train, basic_tensor_train = build_tensor_tqdm_gpt(train_data)
    data_tensor_val, label_tensor_val, basic_tensor_val = build_tensor_tqdm_gpt(val_data)
    data_tensor_test, label_tensor_test, basic_tensor_test = build_tensor_tqdm_gpt(test_data)

    return data_tensor_train, label_tensor_train, basic_tensor_train, data_tensor_val, label_tensor_val, basic_tensor_val, data_tensor_test, label_tensor_test, basic_tensor_test
...
def run_step6_label_1():
    ...
    for i in [20, 24, 30, 36, 48]:
        ...
        (data_tensor_train, label_tensor_train, basic_tensor_train,
         data_tensor_val, label_tensor_val, basic_tensor_val,
         data_tensor_test, label_tensor_test, basic_tensor_test) \
            = splited_tensor(data, 0.8, 0.1)

        data_tensor, label_tensor, basic_tensor = build_tensor_tqdm_gpt(data)

        file = f"data_tensor_{i}.pth"
        torch.save({'data_tensor_train': data_tensor_train,
                    'label_tensor_train': label_tensor_train,
                    'basic_tensor_train': basic_tensor_train,
                    'data_tensor_val': data_tensor_val,
                    'basic_tensor_val': label_tensor_val,
                    'label_tensor_val': basic_tensor_val,
                    'data_tensor_test': data_tensor_test,
                    'label_tensor_test': label_tensor_test,
                    'basic_tensor_test': basic_tensor_test,
                    'data_tensor_cell': data_tensor,
                    'label_tensor_cell': data_tensor,
                    'basic_tensor_cell': basic_tensor}, file)
