...
def fill_isnan(data):
    """缺失值按照ID。先向后填充，再向前填充"""
    columns = ['id', 'time', 'sbp', 'dbp', 'map', 'hr', 'rr', 'spo2', 'temp', 'label', 'gender', 'age']
    dropped_data = pd.DataFrame(data, columns=columns)

    filled_df = dropped_data.groupby('id')[['time', 'sbp', 'dbp', 'map', 'hr', 'rr', 'spo2', 'temp', 'label', 'gender', 'age']].apply(
...
def normalize_data(data):
    ...
    feature_cols = ['sbp', 'dbp', 'map', 'hr', 'rr', 'spo2', 'temp', 'age']
    # 初始化 MinMaxScaler 对象
...
def run_step5_label_1():
    ...
    time_list = []
    data_dirs = ["./pure_data/"]  # 定义需要检查的目录列表
    ...
    for i in tqdm([20]):
        ...
        data_row = pd.read_csv(f"./merged_data_{i}_with_basic_char.csv")
